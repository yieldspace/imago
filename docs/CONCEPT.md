...
## Q5. 実機への配布/更新の体験は？
A. **`imago deploy`で一発配布**。K8sのように**アップロード→再起動**を基本とする。**blue‑greenはMVPでは不要（後で検討）**。

## Q6. blue‑greenのヘルス判定は？
A. **（将来）**実行コンポーネントが**ヘルスチェック用WITに適合**していることを前提にする。
...
## Q152. `stop`のデフォルトシグナルは？
A. **SIGINT**。

## Q153. `stop --force`はSIGKILL？
A. **SIGKILLでOK**。

## Q154. `stop`のgraceful待ち時間の場所は？
A. **`limits`内の`shutdown_timeout`等で指定**。

## Q155. `shutdown_timeout`のデフォルトは？
A. **30s**。

## Q156. `shutdown_timeout`超過時の挙動は？
A. **SIGKILLで強制終了**。

## Q157. `shutdown_timeout`はどこに置く？
A. **`limits`内**。

## Q158. `limits`の他のプロセス制限は？
A. **今のところ無し**。

## Q159. `restart_policy`の再試行回数/間隔は？
A. **`max_retries = 3`** など、**回数指定**ができる形にする。

## Q160. `restart_policy`の間隔は？
A. **指数バックオフ**。

## Q161. `restart_policy`の置き場所は？
A. **`runtime`セクション**。

## Q162. ヘルスチェックの間隔/タイムアウトは？
A. **`interval` / `timeout`で指定できる**形にする。

## Q163. ヘルスチェック失敗の許容回数は？
A. **MVPでは不要。あとで検討**。

## Q164. `build/`の転送形式は？
A. **gzip（例: tar.gz）で1ファイル化して送る**。

## Q165. アーカイブ形式の指定は？
A. **gzip（tar.gz）**。

## Q166. アーカイブのファイル名/置き場所は？
A. **送信時にin‑place変換して送る**。ファイルとしては残さない。

## Q167. `deploy`の進捗表示は？
A. **ステップ表示**。

## Q168. `deploy`のステップ（MVP）は？
A. **build → package → upload → apply → restart**（healthcheckはMVP不要）。

## Q169. `upload`後の整合性チェックは？
A. **hash一致で検証**。

## Q170. ハッシュアルゴリズムは？
A. **SHA-256**。

## Q171. `apply`でMVPに必須な処理は？
A. **展開→配置 / manifest登録 / 旧版クリーンアップ**を全部やる（docker compose的）。

## Q172. 旧版クリーンアップのタイミングは？
A. **新版起動前**。

## Q173. 配置先ディレクトリは？
A. **`/etc`配下**（例: `/etc/imago/...`）。

## Q174. `/etc/imago`配下の構成は？
A. **`/etc/imago/<name>/<hash>/`** の世代管理。
