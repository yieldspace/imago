...
2. `constrained_trace_refines` を front door とする、**constrained model checking ベースの一般化された trace refinement**へ引き上げる。
3. 旧 reduction/export 命名が暗黙に背負っていた trust boundary を、**証明書 (`ProofCertificate`) と trust tier**として型に出す。
...
現在の `nirvash` は、README にあるとおり `nirvash` / `nirvash-ir` / `nirvash-lower` / `nirvash-check` / `nirvash-backends` / `nirvash-conformance` / `nirvash-proof` へ分割され、明確な crate split を持つ。README では `nirvash-ir` を backend 非依存の `SpecCore` と位置づけ、`nirvash-check` を explicit / symbolic の typed checker front door、`nirvash-conformance` を witness / runtime binding / refinement assert、`nirvash-proof` を `ProofBundleExporter` / `ProofDischarger` / certificate importer として公開している。  
参照: [R1], [R2]
...
- `TraceRefinementMap` と `constrained_trace_refines` が入り、summary equality 依存だった step-level conformance が trace-level constrained refinement へ拡張された。
- `TrustTier = Exact | CertifiedReduction | ClaimedReduction | Heuristic` が API surface に現れ、exact / certified / claimed / heuristic が区別された。  
  参照: [R3], [R4], [R5]
...
旧 trace refinement surface は step-level relation を持っていたが、`ObservedEvent` は `Action { action, output } | Stutter` に限定されており、trace matching は candidate trace の state length、step length、`loop_start` 一致を要求していた。これは sequential deterministic API の validation には有効だが、hidden step、linearization point、部分観測、部分状態観測、非同期 invoke/return を持つ実装には狭い。  
参照: [R7], [R8], [R9]

### 1.4 trust boundary が still implicit である

`SymmetryReduction` / `StateQuotientReduction` / `PorReduction` は実装上「関数ポインタ + obligation/certificate の束」として扱い、claim export と certificate import/discharge を分離する。current `ProofBundleExporter` は supported fragment の bundle を export し、`ProofDischarger` / importer が `ProofCertificate` へ接続する。したがって旧「Verified」「Sound」命名は current surface では採用しない。  
参照: [R10], [R11]
...
3. `constrained_trace_refines` を generic refinement map と partial observation を扱える trace-level API とする。
...
IR 側には `FairnessDecl::WF { view, action } / SF { view, action }` があるが、authoring surface では `view` を明示する道が薄かった。実装後の current surface では `TemporalSpec::core_fairness()` を authoring 正本とし、typed executable fairness は lowering/backend 内部 detail として `executable_fairness()` へ退避した。
...
explicit backend が必要とする typed executable fairness は AST-native clause から内部再構成し、opaque / stringly / 非 AST-native fairness は fail-closed とする。
...
旧 step-level relation surface では `AuxState` の逐次更新、partial observation、hidden step の扱いが不足していた。そこで `TraceRefinementMap` を導入した。
...
実装後の current surface では restricted relation wrapper は残さず、公開入口を `TraceRefinementMap` と `constrained_trace_refines` に一本化した。
...
TLA+ trace validation の文献は、まさにこの問題を constrained model checking として扱っている。旧 trace matcher が要求していた「長さ一致」「loop_start 一致」「candidate trace と prefix state 一致」は、完全観測・完全同期の特殊ケースにすぎない。  
参照: [R9], [P2]
...
旧 reduction alias 相当の意味は、semantic には次の 2 層へ分けて実装する。
...
これにより reduction の trust 境界は `ClaimedReduction` / `CertifiedReduction` に分離され、旧 reduction alias は final surface では残さない。
...
旧 tier surface は final API では残さず、4 段階の `TrustTier` へ置換する。

#### 根拠

current implementation では claimed reduction と certified reduction を分離し、proof/export 側も `ProofBundleExporter` + `ProofDischarger` + importer で trust boundary を明示する。  
参照: [R10], [R11]
...
current implementation では pretty/export の正本は `ProofBundleExporter` であり、証明書化は `ProofDischarger` / importer が担う。
...
- `TraceRefinementMap` から `ReferenceStateMachine` / `StateMachineTest` を自動生成する
...
- `constrained_trace_refines` から Kani proof harness を生成する
...
本 RFC は大きな再編だが、final landing では互換 shim を残さず新 surface へ統一した。

- `LoweredSpec { core, symbolic_artifacts, executable }` は維持する
- `ObservedEvent::Action | Stutter` は current enum の subset として扱える
- trace refinement の公開入口は `TraceRefinementMap` と `constrained_trace_refines` に限定する
- 旧 reduction alias / legacy reduction API は final state では残さない

## 8. 実装結果

### Phase 1-5: 実装済み

- `LoweredSpec::core()` / `normalized_core()` を追加し、symbolic backend と proof export を `normalized_core()` first に寄せた。
- `TraceRefinementMap`、拡張 `ObservedEvent`、`StateObservation` を追加し、互換 wrapper は削除した。
- `constrained_trace_refines` を `ExplicitCandidate | ExplicitConstrained | SymbolicConstrained` で実装し、partial observation / hidden step / bounded lasso を扱えるようにした。
- `ReductionClaim<T>` / `Certified<T>` / `ProofCertificate` と `TrustTier = Exact | CertifiedReduction | ClaimedReduction | Heuristic` を導入した。
- `ProofBundleExporter` / `ProofDischarger` / Verus・RefinedRust importer を導入し、`proptest` / Kani / loom adapter を同じ refinement model に揃えた。

## 9. 受け入れ基準

本 RFC は、次の条件を満たす形で実装済みである。
...
### 11.3 旧 reduction alias 命名を維持しない
...
- 旧 proof exporter 命名は rename ではなく `ProofBundleExporter` / `ProofDischarger` へ破壊的に置換する。
...
- [R5] `crates/nirvash-conformance/src/lib.rs`: `TraceRefinementMap` と `constrained_trace_refines`  
...
- [R7] `crates/nirvash-conformance/src/lib.rs`: `ObservedEvent` が `Invoke | Return | Internal | Action | Stutter` を扱う  
  <https://github.com/yieldspace/imago/blob/codex/tla-formal-controls/crates/nirvash-conformance/src/lib.rs>
- [R8] `crates/nirvash-conformance/src/lib.rs`: 旧 trace matcher の shape / loop 一致前提  
  <https://github.com/yieldspace/imago/blob/codex/tla-formal-controls/crates/nirvash-conformance/src/lib.rs>
...
- [R10] `crates/nirvash-lower/src/lib.rs`: `SymmetryReduction` / `StateQuotientReduction` / `PorReduction`  
  <https://github.com/yieldspace/imago/blob/codex/tla-formal-controls/crates/nirvash-lower/src/lib.rs>
- [R11] `crates/nirvash-proof/src/lib.rs`: `ProofBundleExporter` / `ProofDischarger` と supported fragment 制約  
