...
このリポジトリの cargo-deny は workspace 前提です。CI では次の 2 系統に分離しています。

- blocking: `cargo deny --workspace check bans licenses sources`
- advisories: `continue-on-error: true` の job で `cargo deny fetch db` 後に `cargo deny --workspace check -W unmaintained advisories --disable-fetch`
