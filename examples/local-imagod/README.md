...
`imago.toml` の `remote = "ssh://localhost?socket=/tmp/imagod-local.sock"` と `imagod.toml` の `control_socket_path` を一致させ、同じユーザーからその socket に接続できる状態にしてください。
...
- `imago.toml` の `remote = "ssh://localhost?socket=/tmp/imagod-local.sock"` と `imagod.toml` の `control_socket_path` が一致している
